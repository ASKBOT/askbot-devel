...
* Optimized instant email notifications: replaced per-recipient iteration
  with query sets and batched celery tasks to avoid OOM eviction and
  task timeout issues
...
* Use ``EMAIL_VALIDATION_REQUIRED`` setting to to enable/disable
  the requirement for the users to validate their email address
